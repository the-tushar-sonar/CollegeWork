...
int main()
{
    ...
    while (completed < n)
    {
        // Find process with highest priority among available (lower number = higher priority)
        int idx = -1;
        ...
        if (idx == -1)
        {
            ...
            for (int i = 0; i < n; ++i)
                if (!p[i].done)
                    next_arrival = min(next_arrival, p[i].arrival);
            current_time = max(current_time + 1, next_arrival);
            continue;
        }

        // Execute the chosen process completely
        int start_time = current_time;
        current_time += p[idx].burst;
        p[idx].completion = current_time;
        p[idx].turnaround = p[idx].completion - p[idx].arrival;
        p[idx].waiting = p[idx].turnaround - p[idx].burst;
        p[idx].response = p[idx].waiting; // same as waiting time for non-preemptive
    ...
    // Calculate averages
    float total_wt = 0, total_tat = 0, total_rt = 0, total_pr = 0, total_time = 0;
    for (auto &proc : p)
    {
