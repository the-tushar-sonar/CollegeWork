...
mem_page mmap (NULL, PAGE_SIZE, PROT_READ | PROT_WRITE, MAP_ANON | MAP PRIVATE, -1, 0);

if (mem page MAP FAILED) return 2;

// Initialize one big free chunk covering entire page
chunk count = 1;
chunks[0].offset = 0;
chunks[0].size PAGE SIZE;
chunks [0].used = 0;
...
static void split chunk(size t idx, size t size) {
     if (chunk_count + 1 > MAX CHUNKS)     return; // No space for new chunk
...
for(size tj=i+1; j + 1 chunk count; j++) {
    chunks [j] = chunks [j+1];
}
chunk count--;
} else {
...
char* alloc(int size) {
if(size <= 0 || (size % ALIGNMENT) != 0 || !mem page) return NULL;
size = align8(size);
...
size_t offset = ptr (char*)mem_page;
if(offset >= PAGE SIZE) return 2;
for(size_t i=0; i < chunk_count; i++) {
