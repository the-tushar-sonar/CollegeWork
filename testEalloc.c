...
for(int i=0; i < 4; i++) { a[i] = alloc(4096);
...
int mismatch=0; for(int i=0; i < 4; i++) {

//read each chunk for(int j=0; j < 4096; j++)
...
if(mismatch) { } printf("ERROR: Chunk contents did not match\n"); exit(1);

for(int i = 0 ; i < 64 i++) { dealloc(b[i]);

}

printvsz ("should not change:"); printf("Test2: complete\n\n");
...
//write to chunk for(int j=0; j < 4096; j++) *(c[i]+j) = 'c';
